...
func matchesImage(imageClient internalapi.ImageManagerService, image string, containerImage string,
	runtimeHandler string) (bool, error) {
	if image == "" {
		return true, nil
	}
	r1, err := ImageStatus(imageClient, image, false, runtimeHandler)
	if err != nil {
		return false, err
	}
	r2, err := ImageStatus(imageClient, containerImage, false, runtimeHandler)
	if err != nil {
		return false, err
	}
...
func getRepoImage(imageClient internalapi.ImageManagerService, image string, runtimeHandler string) (string, error) {
	r, err := ImageStatus(imageClient, image, false, runtimeHandler)
	if err != nil {
		return "", err
	}
